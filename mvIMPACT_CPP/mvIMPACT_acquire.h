...
{
public:
    explicit EValIDOutOfBounds( const std::string& msg, const std::string& errorOrigin, bool boMsgIsNameOnly = true ) : EProperty( boMsgIsNameOnly ? "Value ID out of bounds for " + msg : msg, errorOrigin, PROPHANDLING_PROP_VAL_ID_OUT_OF_BOUNDS ) {}
...
protected:
        ...
        int param2 = 0 ) const
    {
        ...
        const std::string resultString( pStr ? pStr : "" );
        delete [] pStr;
        return resultString;
    }
...
public:
        ...
        int index = 0 ) const
    {
        ...
        const std::string translationString( pTranslationString );
        delete [] pTranslationString;
...
PYTHON_ONLY( ENUM_PROPERTY( PropertyIPolarizedDataExtractionMode, EnumPropertyI, mvIMPACT::acquire::TPolarizedDataExtractionMode ) )
...
PYTHON_ONLY( ENUM_PROPERTY( PropertyIPolarizedDataExtractionInterpolationMode, EnumPropertyI, mvIMPACT::acquire::TPolarizedDataExtractionInterpolationMode ) )
...
public:
    ...
    std::string readBinary(
        ...
        int index = 0 ) const
    {
        const unsigned int bufSize = binaryDataBufferSize( index );
        if( bufSize == 0 )
        {
            return std::string( "" );
        }
        ...
        const TPROPHANDLING_ERROR result = OBJ_GetBinary( m_hObj, pBuf, bufSize, index );
        if( result != PROPHANDLING_NO_ERROR )
        {
            delete [] pBuf;
            ExceptionFactory::raiseException( MVIA_FUNCTION, __LINE__, result, m_hObj );
        }
        const std::string data( pBuf, bufSize );
        delete [] pBuf;
        return data;
    }
    /// \brief Writes one value to the property.
...
public:
     ...
     *  including sequencer set and user set (see SFNC for details) data by specifying the \b ::sfProcessGenICamSequencerData and/or \b ::sfProcessGenICamUserSetData
     ...
     *
     *  \warning There has been an incompatible change when loading settings in version 2.9.0 and 2.11.0 of mvIMPACT Acquire as well as in version 2.28.0. See <b>mvIMPACT::acquire::FunctionInterface::loadSetting</b> for details.
     *
     *  \return
     *  - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
     *  - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR or \b mvIMPACT::acquire::TPROPHANDLING_ERROR otherwise.
...
#       endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
        ...
        DigitalOutput* pOutput ) const
    {
        ...
        return p ? p->mode.read() : ddomManual;
    }
...
#       endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
        ...
        int index = 0 ) const
    {
        ...
        return p ? p->digitalSignal.getTranslationDictValue( index ) : dsNone;
    }
