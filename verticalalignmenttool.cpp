...
void VerticalAlignmentTool::Evaluate_vignette(){
    ...
    for(int i=0;i<25;i++){
        ...
        cv::Mat image_gray_2;
        cv::threshold(image_gray,image_gray_2,threshold_value,255,CV_THRESH_BINARY_INV);
        ...
        cv::RotatedRect fitted_ellipse = cv::fitEllipseDirect(Centers);
        double eccentricity = 0.;
        EvaluateEccentricity(fitted_ellipse,eccentricity);
        std::cout<<"It. : "<<i<<" thr. :"<<threshold_value<<",eccentricity: "<<eccentricity<<std::endl;
        //        for(unsigned  int j=0; j< contours.size(); j++ )
        //        {
        ...
        cv::ellipse(RoiImage, fitted_ellipse, cv::Scalar(0,0,threshold_value), 3);
    }
